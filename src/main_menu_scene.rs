...
use crate::{scenes::{Scene, SceneSignal, SceneType}, systems::does_save_exist};
...
impl Scene for MainMenuScene {
    fn tick(&mut self, ctx: &mut BTerm) -> SceneSignal {
        ...
        match self.read_input(ctx) {
            MainMenuResult::NoSelection { selected } => { self.selection = selected; SceneSignal::None }
...
impl MainMenuScene {
    pub fn new() -> MainMenuScene {
        MainMenuScene { selection: MainMenuSelection::NewGame, save_exists: does_save_exist() }
    }
